...
compute-mfcc-feats --verbose=2 --sample-frequency=8000 $options 
   scp:material/$target.wav.scp 
   ark,t,scp:$path/$target.13.ark,$path/$target.13.scp 2> $log
add-deltas 
   ark:$path/$target.13.ark 
   ark:$path/$target.tmp.ark 2> $log
compute-cmvn-stats 
   ark:$path/$target.tmp.ark 
   ark:$path/$target.tmp.compute 2> $log
apply-cmvn 
   ark:$path/$target.tmp.compute 
   ark:$path/$target.tmp.ark 
   ark,t,scp:$path/$target.39.cmvn.ark,$path/$target.39.cmvn.scp 2> $log
