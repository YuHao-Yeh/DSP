...
int main(int argc, char *argv[])
{
   ...
   for (int l = 0; l < test.size(); l++)
   {
      ...
      for (int curr = 0; curr < str.at(1).size(); curr++)
      {
         if (str.at(0).size() == 1)
         {
            prob.at(0)[curr] = prob.at(0)[0] + getBigramProb(str.at(0)[0].c_str(), str.at(1)[curr].c_str());
            continue;
         }
      ...
      for (int t = 2; t < len; t++)
      {
         ...
         {
            vector<vector<double>> temp(str.at(t - 1).size(), vector<double>(str.at(t - 2).size(), 0.0));
            for (int prev1 = 0; prev1 < str.at(t - 1).size(); prev1++)
               for (int prev2 = 0; prev2 < str.at(t - 2).size(); prev2++)
                  temp.at(prev1)[prev2] = prob.at(t - 1)[prev1] + getTrigramProb(str.at(t - 1)[prev2].c_str(), str.at(t - 1)[prev1].c_str(), str.at(t)[curr].c_str());

            vector<int> tmp;
            // create 1-dimensional array to find the max element
            for (int i = 0; i < temp.size(); i++)
               tmp.insert(tmp.end(), temp[i].begin(), temp[i].end());

            // get the row and column location of the elment
            int row = (max_element(tmp.begin(), tmp.end()) - tmp.begin()) / temp.size();
            int col = (max_element(tmp.begin(), tmp.end()) - tmp.begin()) % temp.size();
            double max = *max_element(tmp.begin(), tmp.end());
            int index = row;
